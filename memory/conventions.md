# Project Conventions

> Project-specific patterns that emerged over time and are now binding.
> One entry per pattern: WHAT + one-line WHY. Agents: read before tasks,
> append when a new pattern is agreed (memory protocol in AGENTS.md).
> If an entry grows into an architectural choice, promote it to an ADR.

<!-- Example format:
## Forms
All forms use react-hook-form + zod resolver — keeps validation identical
client/server (see ADR-0004).
-->

_No conventions recorded yet._
